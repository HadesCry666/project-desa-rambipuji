...
/* Menghilangkan scrollbar background saat modal Bootstrap aktif */
body.modal-open {
    overflow: hidden !important;
    padding-right: 0 !important;
}

/* Opsional: Sembunyikan visual scrollbar tanpa merusak fungsi scroll */
::-webkit-scrollbar {
    width: 0px;
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-footer bg-white py-3 px-4">
                ...
                <button type="button" class="btn btn-danger btn-rounded px-4" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#modalTolakAdmin-{{ $r->id_pengajuan }}">Tolak</button>
                <button type="button" class="btn btn-success btn-rounded px-4" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#modalSetujuiAdmin-{{ $r->id_pengajuan }}">Setuju</button>
            </div>
        </div>
    </div>
</div>         
        ...
        <div class="modal-content">
            <form action="{{ route('pengajuan.setuju', $r->id_pengajuan) }}" method="POST">
                ...
                <div class="modal-header py-3">
                    <h5 class="modal-title fw-bold">Setujui & Berikan Keterangan Surat</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <!-- Mengubah p-4 menjadi p-3 agar padding modal lebih ringkas -->
                <div class="modal-body p-3">
                    <!-- Mengubah mb-4 menjadi mb-3 -->
                    <div class="card border-0 shadow-sm rounded-3 p-3 mb-3">
                        <div class="mb-2"><span class="text-muted small d-block">Pemohon</span><strong>{{ $r->nama_lengkap }}</strong></div>
                        <div class="mb-2"><span class="text-muted small d-block">NIK</span><code style="font-size: 120%">{{ $r->nik }}</code></div>
                        <div class="mb-2"><span class="text-muted small d-block mb-1">Jenis Surat</span><span class="badge bg-primary">{{ $r->nama_surat }}</span></div>
                        <div class="mb-0"><span class="text-muted small d-block mb-1">Keperluan</span><span>{{ $r->keperluan }}</span></div>
                    </div>

                    {{-- KETERANGAN ADMIN — WAJIB --}}
                    <!-- Mengubah mb default dengan menambah mb-3 -->
                    <div class="card border-warning border-opacity-50 rounded-3 p-3 mb-3">
                        <h6 class="fw-bold mb-1">Keterangan Admin <span class="text-danger">*</span></h6>
                        <p class="text-muted small mb-2">Wajib diisi sebelum surat dapat diteruskan ke Sekretaris Desa.</p>
                        
                        <textarea class="form-control rounded-3" id="keterangan-{{ $r->id_pengajuan }}" name="keterangan_admin" rows="3" placeholder="Tuliskan keterangan surat di sini..." required></textarea>
                    </div>

                    <!-- Menghapus jarak berlebih di bagian bawah alert -->
